...
def video():
    # return Response(generate_frames(path_x='static/files/bikes.mp4'),
    # mimetype='multipart/x-mixed-replace; boundary=frame')
    print("video")
    print(session.get('video_path', None))
...
def webapp():
    # return Response(generate_frames(path_x = session.get('video_path', None),
    # conf_=round(float(session.get('conf_',None))/100,2)),mimetype='multipart/x-mixed-replace; boundary=frame')
